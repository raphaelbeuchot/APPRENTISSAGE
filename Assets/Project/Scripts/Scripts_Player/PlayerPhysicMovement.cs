...
public class PlayerPhysicsMovement : MonoBehaviour
{
    ...
    void HandleInput()
    {
        ...
        // Multiplier par la magnitude originale pour garder l'intensit� du stick
   ...
   IEnumerator SweepCoroutine()
{
    ...
    void EnterCrouch()
    {
        ...
        // GARDE ET CORRIGE le collider (d�tection sentinelle)
        if (capsuleCollider != null)
        {
    ...
    // === FREEZE FUNCTIONS ===
    private Color originalColor; // <--- ajoute cette ligne dans tes variables priv�es
    ...
    void StartFreeze()
    {
        ...
        if (playerRenderer != null)
        {
            // On sauvegarde la couleur du mat�riau (pas le mat�riau lui-m�me)
    ...
    void EndFreeze()
    {
        ...
        if (playerRenderer != null)
        {
            // On restaure simplement la couleur sauvegard�e
    ...
    private bool IsGrounded()
    {
        ...
        Vector3 rayStart = transform.position + Vector3.up * 0.1f; // L�g�rement au-dessus des pieds
    ...
    IEnumerator DashCoroutine(Vector3 direction)
    {
        ...
        while (elapsed < stats.dashDuration)
        {
            ...
            dashVelocity.y = rb.linearVelocity.y; // Garder gravit�
