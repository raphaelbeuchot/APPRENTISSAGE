...
- **✅ Décision de design (2026-09-21, pas encore codée)** : ce n'est pas souhaitable, « il a juste respawn ». **Nouvelle règle : un joueur qui respawne pendant un Red Light est immunisé jusqu'à la fin de ce Red Light, quelle que soit la cause de sa mort.** Implémentation prévue, dans `MultiRespawn.cs` uniquement (fichier multi, sans impact solo, pas de référence de revert à noter) : `respawnImmune = cycle != null && cycle.IsInRedLight()` à la ligne 113 ; retirer le suivi `diedInRedLight` et son relevé dans `Update()` ; l'immunité tombe toujours au changement d'état du cycle (`HandleCycleChanged`), donc jamais sur le cycle suivant. **Effet de bord** : le cas « mort par la sentinelle en Red Light, respawn dans un Red Light suivant » devient lui aussi immunisé (cohérent avec la logique, à confirmer). Pas d'exploit : se laisser mourir renvoie au départ. Mettre aussi à jour « Mort et respawn » (règle d'immunité) une fois codé.
- **À relever avant de coder** : le timing exact (mort en vert ou rouge, respawn dans le même rouge ?), le type de pit (lave, eau, vide, ancien `PitZone`), et la console au respawn : un log `[PlayerPit] Exited spline pit` doit apparaître ; s'il manque, l'état est périmé.
- **Correctif envisagé** : `PlayerPitInteractable.ResetPitState()` (état du pit, eau, gravité, ralentissement) appelé par `MultiRespawn` avec les autres remises à zéro, et retrait de l'entrée du joueur dans `SplinePitFillController`. Touche un fichier partagé avec le solo : commit séparé et référence de revert notée. **Pas encore fait.**
