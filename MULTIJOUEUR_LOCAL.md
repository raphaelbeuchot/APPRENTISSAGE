...
- **Option retenue : un petit composant d'input par joueur, séparé de `PlayerInputManager`** (qui n'est pas touché, donc solo protégé). Il s'appuie sur le `PlayerInput` natif (device apparié, validé en Étape 1a) et expose seulement déplacement, sprint, accroupi. `PlayerPhysicsMovement` lira ce composant s'il existe sur le joueur, sinon le singleton comme avant (en solo, aucun composant local : comportement identique).
