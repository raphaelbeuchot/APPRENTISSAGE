...
- **Prochain test/chantier** : décider comment dupliquer `playerDetectionFeedback` et `stunBySentinel` par joueur (probablement : `GameManager` garde une liste/paire de joueurs au lieu d'un seul, `SentinelShooter`/`SentinelDetector` reçoivent quel joueur a été touché pour ne notifier que lui).
